...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-radian-algo-string-detail.ps1

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-radian-algo-string-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-radian-algo-string-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-radian_algo_string_scope-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
