...
`radian-algo-string-scope` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for string analysis with deny and allow fixtures, explainable decision traces, and documented operating limits.
...
The score is deterministic. The current threshold is 174. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 5.
